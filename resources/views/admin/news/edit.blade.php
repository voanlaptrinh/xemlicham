        ...
        <div class="row justify-content-center">
            <div class="col-md-12 ">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('news.update', $news->id) }}" method="POST" class="" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row text-start g-3">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-12">
                                        <div class="text-center justify-content-center align-items-center p-4 p-sm-5 border border-2 border-dashed position-relative rounded-3">
                                            <img id="preview-image" src="{{ $news->image ? asset('storage/' . $news->image) : asset('/images/gallery.svg') }}" class="h-50px" alt="Preview">

                                            <div>
                                                <h6 class="my-2">Upload course image here, or <a href="#!" class="text-primary">Browse</a></h6>
                                                <label style="cursor:pointer;">
                                                    <span> 
                                                        <input class="form-control stretched-link" type="file" name="image" id="image" accept="image/gif, image/jpeg, image/png" onchange="previewImage(event)">
                                                    </span>
                                                </label>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="datetime-local" name="schedule" value="{{$news->schedule}}" id="schedule" class="form-control">
                                    @error('schedule')
                                    <span class="invalid-feedback" role="alert">
                                        <label class="error" id="name_error" for="name">{{ $message }}</label>
                                    </span>
                                ...
                                <div class="form-group">
                                    <label for="schedule">Nội dung:</label>
                                    <textarea id="content" name="content" id="content">{{ $news->content }}</textarea>
                                    @error('content')
                                    <span class="invalid-feedback" role="alert">
                                        <label class="error" id="name_error" for="name">{{ $message }}</label>
                                    </span>
